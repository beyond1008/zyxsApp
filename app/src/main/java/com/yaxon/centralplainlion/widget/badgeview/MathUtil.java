...
class MathUtil {
    ...
    static double getTanRadian(double atan, int quadrant) {
        if (atan < 0) {
    ...
    static int getQuadrant(PointF p, PointF center) {
        if (p.x > center.x) {
            if (p.y > center.y) {
                return 4;
    ...
    static float getPointDistance(PointF p1, PointF p2) {
     ...
     */
    static void getInnertangentPoints(PointF circleCenter, float radius, Double slopeLine, List<PointF> points) {
